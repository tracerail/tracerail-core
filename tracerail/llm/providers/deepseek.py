...
class DeepSeekProvider(BaseLLMProvider):
    ...
    def _prepare_api_request(self, request: LLMRequest) -> Dict[str, Any]:
        """Prepares the request payload for the DeepSeek API."""
    ...
    def _parse_response(self, data: Dict[str, Any], request: LLMRequest) -> LLMResponse:
        """Parses a successful response from the DeepSeek API."""
    ...
    async def process(self, request: LLMRequest) -> LLMResponse:
        """Processes a request using the DeepSeek API."""
        if not self._client:
        ...
        try:
            ...
            raise LLMAPIError(f"DeepSeek API error: {e.response.text}", provider=self.provider_name, original_error=e) from e
        except httpx.RequestError as e:
            raise LLMAPIError(f"Request to DeepSeek failed: {e}", provider=self.provider_name, original_error=e) from e

    async def stream(self, request: LLMRequest) -> AsyncGenerator[LLMResponse, None]:
        """Processes a request as a stream using the DeepSeek API."""
        if not self._client:
            await self.initialize()
        ...
        try:
            ...
            raise LLMAPIError(f"DeepSeek API streaming error: {e.response.text}", provider=self.provider_name, original_error=e) from e
        except httpx.RequestError as e:
            raise LLMAPIError(f"Streaming request to DeepSeek failed: {e}", provider=self.provider_name, original_error=e) from e

    async def get_capabilities(self) -> LLMCapabilities:
        """Returns the static capabilities of the DeepSeek provider."""
        return LLMCapabilities(
            ...
            max_context_length=32768,  # For deepseek-chat
    ...
    async def health_check(self) -> Dict[str, Any]:
        """Performs a health check on the DeepSeek provider."""
        try:
